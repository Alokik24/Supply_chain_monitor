...
def train_and_evaluate(csv_path: str, output_model_dir: str = "models", metrics_path: str = "metrics.json"):
    """
    ...
    # 6. Calculate Metrics
    p_if, r_if, f_if, _ = precision_recall_fscore_support(y_test, iforest_preds, average="binary", zero_division=0)
    p_rf, r_rf, f_rf, _ = precision_recall_fscore_support(y_test, rf_preds, average="binary", zero_division=0)
    ...
    metrics = {
        "isolation_forest": {
            "precision": round(p_if, 4), "recall": round(r_if, 4), "f1_score": round(f_if, 4),
            "confusion_matrix": {"tn": int(tn_if), "fp": int(fp_if), "fn": int(fn_if), "tp": int(tp_if)}
        },
        "random_forest": {
            "precision": round(p_rf, 4), "recall": round(r_rf, 4), "f1_score": round(f_rf, 4),
            "confusion_matrix": {"tn": int(tn_rf), "fp": int(fp_rf), "fn": int(fn_rf), "tp": int(tp_rf)}
        }
    }
