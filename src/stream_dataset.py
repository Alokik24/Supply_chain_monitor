...
async def send_sensor_reading(client: httpx.AsyncClient, payload: dict, semaphore: asyncio.Semaphore):
    """
    ...
    async with semaphore:
        try:
            ...
            if response.status_code == 200:
                ...
                if status == "ignored":
                    logger.debug(f"Idempotent duplicate dropped safely: {payload['sensor_type']}")
                else:
                    logger.debug(f"Telemetry record saved: {payload['sensor_type']}")
            else:
                logger.error(f"API rejection error code {response.status_code}: {response.text}")
    ...
    async with httpx.AsyncClient(limits=limits) as client:
        try:
            with open(CSV_FILE_PATH, mode="r", encoding="utf-8") as file:
                ...
                logger.info("Initializing production telemetry replay engine at 100 rows/sec (300 events/sec)...")
                ...
                for row in reader:
                    ...
                    sensors_to_extract = [
                        ...
                        {"type": "conveyor_speed", "value": float(row["conveyor_speed"])},
                    ...
                    for sensor in sensors_to_extract:
                        ...
                        task = asyncio.create_task(send_sensor_reading(client, payload, semaphore))
                    ...
                    if batch_row_count >= 100:
                        ...
                        batch_start_time = time.time()
                        batch_row_count = 0
                        logger.info(f"Sustained pipeline progress tracking: Sent {total_rows_processed} wide rows.")
                
                # FIX: Final Flush Sequence
                # Await any remaining tasks left in the array after the file reader finishes
                if active_tasks:
                    logger.info(f"File parsing complete. Draining final {len(active_tasks)} sensor events...")
                    await asyncio.gather(*active_tasks)
                    active_tasks.clear()
                
                logger.info(f"Stream ingestion run finished. Successfully committed {total_rows_processed} wide rows.")
                
        except FileNotFoundError:
            logger.critical(f"Target simulation file missing at layout coordinate path: {CSV_FILE_PATH}")
