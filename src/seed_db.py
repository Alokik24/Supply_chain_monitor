...
def get_database_connection():
    ...
    print(
        f"Connecting to infrastructure pool at: postgresql://{user}:****@{host}:{port}/{db_name}"
    )
...
def seed_telemetry_data(engine):
    ...
    print(
        f"Staging database transaction: committing {len(batch_records)} narrow entries..."
    )
    ...
    print(
        f"Success! Relational database populated with {len(df)} wide events ({len(batch_records)} rows)."
    )
...
def run_verification_metrics(engine):
    ...
    with engine.connect() as connection:
        ...
        for row in results:
            print(
                f" Sensor Type: {row[0]:<15} | Rows Inserted: {row[1]:<5} | Calculated Mean: {row[2]}"
            )
