...
### B. Conveyor Speed Stream ($S$)
Models the physical linear velocity of the packaging line. It contains a low-frequeny sinusoidal pattern modeling natural factory temperature shifts and motor efficiency cycles throughout the day.
* **Baseline ($\mu$):** $1200\text{ RPM}$
* **Cyclic Component:** $10 \times \sin(\text{time\_index} / 60)$
* **Noise ($N$):** Gaussian White Noise sampled from $\mathcal{N}(\mu=0, \sigma=75)$
...
Anomalies are not injected as isolated random spikes. Instead, they simulate a **Correlated Physical Event** where one or more data streams break normal statistical boundaries acording to sensro specific activation probabilities:
...
To avoid creating trivially separable synthetic anomalies, three realism mechanisms are applied:

### Partial Sensor Activation

Each sensor responds independently with:

- Torque: 85%
- Speed: 80%
- Fill: 70%

This models sensor lag and partial fault propagation.

### Magnitude Jitter

Anomaly magnitudes receive a random ±20% scaling factor.

This prevents all failures from exhibiting identical severity.

### Temporal Bleed

A fault at time t propagates residual effects to:

- t+1 (55% intensity)
- t+2 (25% intensity)

These rows remain labelled as normal observations.

This forces downstream models to distinguish between true failures and post-fault recovery behavior.