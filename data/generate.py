...
def generate_hardware_wide_data():
    ...
    for idx in anomaly_indices:
        ...
        if fires_torque:
            torque[idx] += torque_delta
        if fires_speed:
            speed[idx] -= speed_delta
        if fires_fill:
            fill_level[idx] -= fill_delta
        ...
        for lag, decay in enumerate(BLEED_DECAY, start=1):
            ...
            if bleed_idx >= records:
                break
            if fires_torque:
                torque[bleed_idx] += torque_delta * decay
            if fires_speed:
                speed[bleed_idx] -= speed_delta * decay
            if fires_fill:
                fill_level[bleed_idx] -= fill_delta * decay
    ...
    clean_indices = np.array(
        [i for i in range(records) if i not in anom_pos and i not in bleed_pos]
    )
    ...
    print(
        f"  Anomaly events     : {int(is_anomaly.sum()):,}  ({actual_pct:.2f}% of total)"
    )
    ...
    for col, lbl, sigma in [
        ...
        print(
            f"    {lbl}  μ={vals.mean():8.3f}   σ={vals.std():.3f}  (design σ={sigma})"
        )
