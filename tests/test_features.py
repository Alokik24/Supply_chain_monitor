...
def test_build_feature_matrix_shape_and_columns(narrow_df):
    ...
    expected_metrics = [
        "conveyor_speed_rolling_mean",
        "conveyor_speed_rolling_std",
        "conveyor_speed_z_score",
        "conveyor_speed_rate_of_change",
        "conveyor_speed_delta_from_baseline",
        "torque_z_score",
        "fill_level_z_score",
    ]
...
def test_feature_pipeline_empty_input_boundary():
    ...
    """
    empty_narrow_df = pd.DataFrame(
        columns=["id", "line_id", "sensor_type", "value", "timestamp"]
    )

    with pytest.raises(
        ValueError, match="Cannot build feature matrix from empty telemetry DataFrame"
    ):
    ...
    scrambled_df = pd.DataFrame(
        {
            ...
            "value": [200.0, 100.0],
            "timestamp": pd.to_datetime(
                ["2026-01-01 00:05", "2026-01-01 00:01"]
            ),  # Newest row listed first
        }
    )
...
def test_cross_factory_line_isolation():
    ...
    multi_line_df = pd.DataFrame(
        {
            ...
            "value": [
                ...
                900.0,
                910.0,
            ],  # Line 2 operates at a vastly higher numeric scope
    ...
    line_1_results = feature_matrix[feature_matrix["line_id"] == "Line_1"].reset_index(
        drop=True
    )
    line_2_results = feature_matrix[feature_matrix["line_id"] == "Line_2"].reset_index(
        drop=True
    )
...
def test_compute_delta_from_baseline_isolated(narrow_df):
    ...
    assert (
        pytest.approx(processed_df.loc[0, "conveyor_speed_delta_from_baseline"])
        == 150.0
    )

    # Row index 1 baseline drift variance = 900.0 - 1050.0 = -150.0
    assert (
        pytest.approx(processed_df.loc[1, "conveyor_speed_delta_from_baseline"])
        == -150.0
    )
